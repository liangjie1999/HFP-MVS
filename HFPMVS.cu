...
// 获取从origin看向pixel对应3d点的view direction
...
// 根据一个像素点得到其在相机坐标系下的坐标，然后计算其在平面法向量方向的投影（其实该投影即坐标原点到平面的距离）
//                                                                      该投影可用于计算单应矩阵（公式中的d）
...
// 计算patch similairty 时 计算某点的权重（与该点距中心点距离，该点与中心点颜色差异相关）
...
// 计算与所有视图的Bilateral NCC
__device__ void ComputeMultiViewCostVector(const cudaTextureObject_t *images, const Camera *cameras, const int2 p, const float4 plane_hypothesis, float *cost_vector, const PatchMatchParams params, uint8_t* region = nullptr)
{
    for (int i = 1; i < params.num_images; ++i) {
...
__device__ void ComputeCannotDepth(const Camera* cameras, int2 p, const PatchMatchParams params, float4* plane_hypotheses, uint8_t* region, float* cannot_depth) {
    int width = cameras[0].width;
    int height = cameras[0].height;

    int size = params.patch_size;
    int num = size * size;
    int radius = size / 2;

    if(p.x <= radius - 1 || p.y <= radius -1 || p.x >= (width - radius) || p.y >= (height - radius) || region[p.y * width + p.x] == 0) {
        return ;
    }
    int center_idx = p.y * width + p.x;
    uint8_t cur_region_id = region[center_idx];
    {
        int cannot_depth_idx = 0;
        int cannot_depth_num = 0;
        // 左上角 (p.x - radius, p.y - radius) 右下角 (p.x + radius, p.y + radius)
        for(int i = p.x - radius; i < (p.x + radius); i++) {
            for(int j = p.y - radius; j < (p.y + radius); j++) {
                if(cannot_depth_num >= 3) {
                    break;
                }

                int temp_idx = j * width + i;
                uint8_t temp_region_id = region[temp_idx];

                if(temp_region_id != 0 && temp_region_id != cur_region_id) {
                    float cur_depth = ComputeDepthfromPlaneHypothesis(cameras[0], plane_hypotheses[temp_idx], make_int2(i, j));                    
                
                    bool is_same = false;

                    for(int k = 0; k < cannot_depth_num; k++) {
                        if(cannot_depth[k] != -1.0f && abs(cur_depth - cannot_depth[k]) < 0.04) {
                            is_same = true;
                            break;
                        }
                    }
                    ...
                    if(!is_same) {
                        cannot_depth[cannot_depth_idx] = cur_depth;
                        cannot_depth_idx++;
                        cannot_depth_num++;
                    }
                }
...
__device__ bool is_cannot_depth(const Camera camera, int2 center, int src_idx, float4* plane_hypotheses, float* cannot_depth) {
    float src_depth = ComputeDepthfromPlaneHypothesis(camera, plane_hypotheses[src_idx], center);
    for(int i = 0; i < 3; i++) {
        if(cannot_depth[i] > 0.1f && (abs(src_depth - cannot_depth[i]) < 0.08)) {
            return true;
        }
    }
...
{   
    ...
    const int center = p.y * width + p.x;
    float cannot_depth[3] = {-0.1f, -0.1f, -0.1f};
    // if(params.cannot_depth && !not_freeze[center]) {
    //     return ;
    // }
    ...
    // if (not_freeze[center]) {
    //     ComputeCannotDepth(cameras, p, params, plane_hypotheses, region, cannot_depth);
    // } 
    ...
    uint8_t region_c = region[center];
    bool p_region_pro = params.region_pro;
    if(region_c == 0) {
        p_region_pro = false;
    }
    ...
    if (p.y > 2) {
        ...
        if(is_cannot_depth(cameras[0], p, up_far, plane_hypotheses, cannot_depth)) {
            costMin += cannot_cost;
        } 
        ...
        for (int i = 1; i < 11; ++i) {
            if (p.y > 2 + 2 * i) {
                ...
                float cost = is_cannot_depth(cameras[0], p, pointTemp, plane_hypotheses, cannot_depth) ? costs[pointTemp] + cannot_cost : costs[pointTemp];
                if (cost < costMin && (!p_region_pro || region[pointTemp] == region_c)) {
                    costMin = cost;
        ...
        if(p_region_pro && region[costMinPoint] != region_c) {
            flag[1] = false;
        } else {
            up_far = costMinPoint;
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_far], cost_array[1], params, region);
        }
    } 
    // down_far
    if (p.y < height - 3) {
        ...
        if(is_cannot_depth(cameras[0], p, down_far, plane_hypotheses, cannot_depth)) {
            costMin += cannot_cost;
        } 
        for (int i = 1; i < 11; ++i) {
            if (p.y < height - 3 - 2 * i) {
        ...
        if(p_region_pro && region[costMinPoint] != region_c) {
            ...
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_far], cost_array[3], params, region);
        }
    } 
    // left_far
    if (p.x > 2) {
        ...
        if(is_cannot_depth(cameras[0], p, left_far, plane_hypotheses, cannot_depth)) {
            costMin += cannot_cost;
        }
        ...
        if(p_region_pro && region[costMinPoint] != region_c) {
            ...
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_far], cost_array[5], params, region);
        }
    } 
    // right_far
    if (p.x < width - 3) {
        ...
        if(is_cannot_depth(cameras[0], p, right_far, plane_hypotheses, cannot_depth)) {
            costMin += cannot_cost;
        } 
        for (int i = 1; i < 11; ++i) {
            if (p.x < width - 3 - 2 * i) {
        ...
        if(p_region_pro && region[costMinPoint] != region_c) {
            ...
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_far], cost_array[7], params, region);
        }
    ...
    if (p.y > 0) {
        ...
        if(is_cannot_depth(cameras[0], p, up_near, plane_hypotheses, cannot_depth)) {
            costMin += cannot_cost;
        } 
        for (int i = 0; i < 3; ++i) {
            if (p.y > 1 + i && p.x > i) {
        ...
        if(p_region_pro && region[costMinPoint] != region_c) {
            ...
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[up_near], cost_array[0], params, region);
        }
    }
    // printf("Start 8\n");
    // // down_near
    if (p.y < height - 1) {
        ...
        if(is_cannot_depth(cameras[0], p, down_near, plane_hypotheses, cannot_depth)) {
            costMin += cannot_cost;
        } 
        for (int i = 0; i < 3; ++i) {
        ...
        if(p_region_pro && region[costMinPoint] != region_c) {
            ...
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[down_near], cost_array[2], params, region);
        }
    }
    // printf("Start 9\n");
    // left_near
    if (p.x > 0) {
        ...
        if(is_cannot_depth(cameras[0], p, left_near, plane_hypotheses, cannot_depth)) {
            costMin += cannot_cost;
        }
        for (int i = 0; i < 3; ++i) {
            if (p.x > 1 + i && p.y > i) {
        ...
        if(p_region_pro && region[costMinPoint] != region_c) {
            ...
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[left_near], cost_array[4], params, region);
        }
    }
    // printf("Start 10\n");
    // // right_near
    if (p.x < width - 1) {
        ...
        if(is_cannot_depth(cameras[0], p, right_near, plane_hypotheses, cannot_depth)) {
            costMin += cannot_cost;
        }
        for (int i = 0; i < 3; ++i) {
        ...
        if(p_region_pro && region[costMinPoint] != region_c) {
            ...
            ComputeMultiViewCostVector(images, cameras, p, plane_hypotheses[right_near], cost_array[6], params, region);
        }
    }
    ...
    // 根据四个邻居点在32张视图下是否被选中过 来决定当前点在32张视图下是否应该被选中的先验概率
...
__global__ void BlackPixelUpdate(cudaTextureObjects *texture_objects, cudaTextureObjects *texture_depths, Camera *cameras, float4 *plane_hypotheses, float *costs,  float *pre_costs,  curandState *rand_states, unsigned int *selected_views, float4 *prior_planes, unsigned int *plane_masks, const PatchMatchParams params, const int iter, uint8_t* region_cuda, uint8_t* not_freeze = nullptr)
{
    ...
    CheckerboardPropagation(texture_objects[0].images, texture_depths[0].images, cameras, plane_hypotheses, costs, pre_costs,  rand_states, selected_views, prior_planes, plane_masks, p, params, iter, region_cuda, not_freeze);
}

__global__ void RedPixelUpdate(cudaTextureObjects *texture_objects, cudaTextureObjects *texture_depths, Camera *cameras, float4 *plane_hypotheses, float *costs,  float *pre_costs, curandState *rand_states, unsigned int *selected_views, float4 *prior_planes, unsigned int *plane_masks, const PatchMatchParams params, const int iter, uint8_t* region_cuda, uint8_t* not_freeze = nullptr)
{
    int2 p = make_int2(blockIdx.x * blockDim.x + threadIdx.x, blockIdx.y * blockDim.y + threadIdx.y);
    if (threadIdx.x % 2 == 0) {
        p.y = p.y * 2 + 1;
...
void HFPMVS::RunPatchMatch()
{
    ...
    for (int i = 0; i < max_iterations; ++i) {
        ...
        //     SetCannotDepth(true);
        // } else {
        //     SetCannotDepth(false);
        // }
        BlackPixelUpdate<<<grid_size_checkerboard, block_size_checkerboard>>>(texture_objects_cuda, texture_depths_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, pre_costs_cuda, rand_states_cuda, selected_views_cuda, prior_planes_cuda, plane_masks_cuda, params, i, region_cuda, not_freeze_cuda);
        CUDA_SAFE_CALL(cudaDeviceSynchronize());
        RedPixelUpdate<<<grid_size_checkerboard, block_size_checkerboard>>>(texture_objects_cuda, texture_depths_cuda, cameras_cuda, plane_hypotheses_cuda, costs_cuda, pre_costs_cuda,rand_states_cuda, selected_views_cuda, prior_planes_cuda, plane_masks_cuda, params, i, region_cuda, not_freeze_cuda);
        CUDA_SAFE_CALL(cudaDeviceSynchronize());
        printf("iteration: %d\n", i);
    }
...
void HFPMVS::findRegionInitialization(std::vector<float4> planeParams_tri, cv::Mat_<float> mask_tri, cv::Mat errors) {
    int rows = mask_tri.rows;
    int cols = mask_tri.cols;
    
    cudaMalloc((void **) &planeParams_tri_cuda, sizeof(float4) * planeParams_tri.size());
    cudaMalloc((void **) &mask_tri_cuda, sizeof(float) * rows * cols);
    cudaMalloc((void **) &errors_cuda, sizeof(uint8_t) * rows * cols);

    float4* planeParams_tri_host = new float4[planeParams_tri.size()];
    mask_tri_host = new float[rows * cols];
    uint8_t* errors_host = new uint8_t[rows * cols];

    for(int i = 0; i < rows; i++) {
        for(int j = 0; j < cols; j++) {
            int idx = i * cols + j;

            mask_tri_host[idx] = mask_tri(i, j);
            errors_host[idx] = errors.at<uint8_t>(i, j);
        }
    }

    for(int i = 0, n = planeParams_tri.size(); i < n; i++) {
        planeParams_tri_host[i] = planeParams_tri[i];
    }

    cudaMemcpy(planeParams_tri_cuda, planeParams_tri_host, sizeof(float4) * planeParams_tri.size(), cudaMemcpyHostToDevice);
    cudaMemcpy(mask_tri_cuda, mask_tri_host, sizeof(float) * rows * cols, cudaMemcpyHostToDevice);
    cudaMemcpy(errors_cuda, errors_host, sizeof(uint8_t) * rows * cols, cudaMemcpyHostToDevice);
    

    delete []planeParams_tri_host;
    delete []errors_host;
}

__device__ uint8_t findRegionOf0(uint8_t* regions, int height, int width, int x, int y) {
    ...
    int top = max(y - conv_size / 2, 0);
    int bottom = min(y + conv_size / 2 + 1, height);
    int left = max(x - conv_size / 2, 0);
    int right = min(x + conv_size / 2 + 1, width);

    int max_area = 0;
    uint8_t max_region_id = 0;
    for(int i = top; i < bottom; i++) {
        for(int j = left; j < right; j++) {
            int cur_idx = i * width + j;
            
            patch_areas[regions[cur_idx]]++;
        
            if(regions[cur_idx] != 0) {
                if(patch_areas[regions[cur_idx]] > max_area) {
                    max_area = patch_areas[regions[cur_idx]];
                    max_region_id = regions[cur_idx];
                }
            }
        }
    }
...
__device__ float GetDepthFromPlaneParam(const float4 plane_hypothesis, const int x, const int y, Camera* cameras)
{   
...
__global__ void findSurRegion(float4 *planeParams_tri, float* mask_tri, uint8_t* errors, uint8_t* regions, int height, int width, float max_depth, float min_depth, Camera* cameras) {
    ...
    if(regions[idx] == 0) {
        region_id = findRegionOf0(regions, height, width, x, y);
    }

    while(step <= max_step) {
        int topRowY = max(0, y - step);

        if(topRowY == y - step) {
            int topRowStartX = max(0, x - step);
            int topRowEndX = min(width - 1, x + step);

            while(topRowStartX <= topRowEndX) {
                int cur_idx = topRowY * width + topRowStartX;
                
                if((int)errors[cur_idx] == 0 && mask_tri[cur_idx] != 0 && regions[cur_idx] == region_id) {
                    float d = GetDepthFromPlaneParam(planeParams_tri[(int)mask_tri[cur_idx] - 1], x, y, cameras);
                   
                    if (d <= max_depth && d >= min_depth) {
                        mask_tri[idx] = mask_tri[cur_idx];
                        return ;
                    }
        ...
        int bottomRowY = min(height - 1, y + step);
        if(bottomRowY == y + step) {
            int bottomRowStartX = max(0, x - step);
            int bottomRowEndX = min(width - 1, x + step);

            while(bottomRowStartX <= bottomRowEndX) {
                int cur_idx = bottomRowY * width + bottomRowStartX;
                if((int)errors[cur_idx] == 0 && mask_tri[cur_idx] != 0 && regions[cur_idx] == region_id) {
                    float d = GetDepthFromPlaneParam(planeParams_tri[(int)mask_tri[cur_idx] - 1], x, y, cameras);
        ...
        int leftColX = max(0, x - step);
        if(leftColX == x - step) {
            int leftColStartY = max(1, y - step + 1);
            int leftColEndY = min(height - 2, y + step - 1);

            while(leftColStartY <= leftColEndY) {
                int cur_idx = leftColStartY * width + leftColX;
                if((int)errors[cur_idx] == 0 && mask_tri[cur_idx] != 0 && regions[cur_idx] == region_id) {
                    float d = GetDepthFromPlaneParam(planeParams_tri[(int)mask_tri[cur_idx] - 1], x, y, cameras);
        ...
        int rightColX = min(width - 1, x + step);
        if(rightColX == x + step) {
            int rightColStartY = max(1, y - step + 1);
            int rightColEndY = min(height - 2, y + step - 1);

            while(rightColStartY <= rightColEndY) {
                int cur_idx = rightColStartY * width + rightColX;
                if((int)errors[cur_idx] == 0 && mask_tri[cur_idx] != 0 && regions[cur_idx] == region_id) {
                    float d = GetDepthFromPlaneParam(planeParams_tri[(int)mask_tri[cur_idx] - 1], x, y, cameras);
...
void HFPMVS::RunFindSurRegion(cv::Mat_<float> &mask_tri) {
    int width = mask_tri.cols;
    int height = mask_tri.rows;  
    ...
    findSurRegion<<<grid, block>>>(planeParams_tri_cuda, mask_tri_cuda, errors_cuda, region_cuda, height, width, GetMaxDepth(), GetMinDepth(), cameras_cuda);
    CUDA_SAFE_CALL(cudaDeviceSynchronize());

    cudaMemcpy(mask_tri_host, mask_tri_cuda, sizeof(float) * width * height, cudaMemcpyDeviceToHost);
    for(int i = 0; i < height; i++) {
        for(int j = 0; j < width; j++) {
            int idx = i * width + j;
            mask_tri(i, j) = mask_tri_host[idx];
        }
    }
