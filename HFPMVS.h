...
struct PatchMatchParams {
    ...
    bool region_pro = false;
    bool region_patch = false;
    
    bool freeze = false;
    float region_cost_ratio = 0.21f;
    float geom_changed = 0.02f;
    bool cannot_depth = true;
};
...
public:
    ...
    void InuputInitialization(const std::string &dense_folder, const std::string &output_folder, const std::vector<Problem> &problem, const int idx);
    void Colmap2MVS(const std::string &dense_folder, std::vector<Problem> &problems);
    void CudaSpaceInitialization(const std::string &dense_folder, const std::string &output_folder, const Problem &problem, const std::string &region_cur);
    void RunPatchMatch();
    ...
    float GetMinDepth();
    float GetMaxDepth();
    void SetCannotDepth(bool value);
    void CudaPlanarPriorInitialization(const std::vector<float4> &PlaneParams, const cv::Mat_<float> &masks);
    void RunFindSurRegion(cv::Mat_<float> &mask_tri);
    void findRegionInitialization(std::vector<float4> planeParams_tri, cv::Mat_<float> mask_tri, cv::Mat errors);
private:
    int num_images;
    ...
    uint8_t *region_cuda;
    float4 *planeParams_tri_cuda;
    float *mask_tri_cuda;
    uint8_t *errors_cuda;
    uint8_t *not_freeze_cuda;
};
