...
HFPMVS::~HFPMVS()
{
    ...
    if (params.planar_prior) {
        ...
        cudaFree(planeParams_tri_cuda);
        cudaFree(mask_tri_cuda);
        cudaFree(errors_cuda);
        delete[] mask_tri_host;
    }

    if (params.freeze) {
        delete[] not_freeze_host;
        cudaFree(not_freeze_cuda);
    }
}

void HFPMVS::SetCannotDepth(bool value = true) {
    params.cannot_depth = value;
}
...
void HFPMVS::SetGeomConsistencyParams(bool multi_geometry=false)
{
...
void HFPMVS::SetHierarchyParams()
{
    params.hierarchy = true;
}

void HFPMVS::SetPlanarPriorParams()
{
    params.planar_prior = true;
}

void HFPMVS::InuputInitialization(const std::string &dense_folder,const std::string &output_folder, const std::vector<Problem> &problems, const int idx)
{
    images.clear();
    cameras.clear();
    ...
    if (params.geom_consistency) {
        ...
        for (size_t i = 0; i < num_src_images; ++i) {
            std::stringstream result_path;
            result_path << output_folder << "/HFPMVS" << "/" << std::setw(8) << std::setfill('0') << problem.src_image_ids[i];
            std::string result_folder = result_path.str();
            std::string depth_path = result_folder + suffix;
            cv::Mat_<float> depth;
...
void HFPMVS::CudaSpaceInitialization(const std::string &dense_folder, const std::string &output_folder, const Problem &problem, const std::string &region_cur)
{
    ...
    if (params.hierarchy) {
        ...
        cudaMalloc((void**)&pre_costs_cuda, sizeof(float) * cameras[0].height * cameras[0].width);
        // QUESTION：有什么情况下是不相等的么？安装代码的调用来说 同一尺寸内，只有第一次调用会设置hierarchy为true
        ...
        for (int col = 0; col < width; ++col) {
            for (int row = 0; row < height; ++row) {
                ...
                plane_hypothesis.y = ref_normal(row, col)[1];
                plane_hypothesis.z = ref_normal(row, col)[2];
                // 如果上采样，则粗尺度w保存的是代价；如果非小采样，则粗尺度w保存的是深度
                if (params.upsample) {
    ...
    cv::resize(region, region, cv::Size(cameras[0].width, cameras[0].height), 0, 0, cv::INTER_NEAREST);
    int rows = region.rows;
    int cols = region.cols;
    region_host = new uint8_t[rows * cols];

    for (int col = 0; col < cols; ++col) {
        for (int row = 0; row < rows; ++row) {
            int center = row * cols + col;
            
            region_host[center] = region.at<uint8_t>(row, col);
        }
    }
    
    cudaMalloc((void**)&region_cuda, sizeof(uint8_t) * cols * rows);
    cudaMemcpy(region_cuda, region_host, sizeof(uint8_t) * cols * rows, cudaMemcpyHostToDevice);

    std::cout << "params.freeze: " << params.freeze << std::endl;
    if (params.freeze) {
        not_freeze_host = new uint8_t[rows * cols];
        
        std::stringstream border_path;
        border_path << dense_folder << "/border/contour/" << problem.ref_image_id << ".jpg";
        
        cv::Mat border;
        border = imread(border_path.str(), cv::IMREAD_GRAYSCALE);
 
        int conv_size = 51;
        int half_conv_size = conv_size / 2;
        int width = cols;
        int height = rows;
        uint8_t* not_freeze_host = new uint8_t[width * height];
        memset(not_freeze_host, 0, width * height);
        for (int i = half_conv_size; i < height - half_conv_size; i++) {
            for (int j = half_conv_size; j < width - half_conv_size; j++) {
                if(border.at<uint8_t>(i, j) == 255) {
                    for (int k = -half_conv_size; k <= half_conv_size; k++) {
                        for (int l = -half_conv_size; l <= half_conv_size; l++) {
                            not_freeze_host[(i + k) * width + j + l] = 255;
                        }
                    }                    
                }
                ...
                // for (int k = -half_conv_size; k <= half_conv_size; k++) {
                //     for (int l = -half_conv_size; l <= half_conv_size; l++) {
                //         if(border.at<uint8_t>(i + k, j + l) == 255) {
                //             has_255 = true;
                //             break;
                //         }
                //     }
                ...
                // }
                // if (has_255) {
                //     not_freeze_host[i * width + j] = 255;
                // } else {
                //     not_freeze_host[i * width + j] = 0;
                // }
            }
        } 
        cudaMalloc((void **)&not_freeze_cuda, sizeof(uint8_t) * cols * rows);
        cudaMemcpy(not_freeze_cuda, not_freeze_host, sizeof(uint8_t) * cols * rows, cudaMemcpyHostToDevice);
    }
}

void HFPMVS::CudaPlanarPriorInitialization(const std::vector<float4> &PlaneParams, const cv::Mat_<float> &masks)
{
...
int HFPMVS::GetReferenceImageWidth()
{
    return cameras[0].width;
}

int HFPMVS::GetReferenceImageHeight()
{
    return cameras[0].height;
}

cv::Mat HFPMVS::GetReferenceImage()
{
    return images[0];
}

float4 HFPMVS::GetPlaneHypothesis(const int index)
{
    return plane_hypotheses_host[index];
}

float HFPMVS::GetCost(const int index)
{
    return costs_host[index];
}

float HFPMVS::GetMinDepth()
{
    return params.depth_min;
}

float HFPMVS::GetMaxDepth()
{
    return params.depth_max;
}

void HFPMVS::GetSupportPoints(std::vector<cv::Point>& support2DPoints)
{
...
std::vector<Triangle> HFPMVS::DelaunayTriangulation(const cv::Rect boundRC, const std::vector<cv::Point>& points)
{
    if (points.empty()) {
...
float4 HFPMVS::GetPriorPlaneParams(const Triangle triangle, const cv::Mat_<float> depths)
{
...
float HFPMVS::GetDepthFromPlaneParam(const float4 plane_hypothesis, const int x, const int y)
{
...
void RunJBU(const std::string &output_folder, const cv::Mat_<float>  &scaled_image_float, const cv::Mat_<float> &src_depthmap, const std::string &dense_folder , const Problem &problem)
{
