...
void ProcessProblem(const std::string &dense_folder, const std::string &output_folder, const std::vector<Problem> &problems, const int idx, bool geom_consistency, bool planar_prior, bool hierarchy, const std::string &region_cur, bool multi_geometrty=false)
{   
    const Problem problem = problems[idx];
    std::cout << "Processing image " << std::setw(8) << std::setfill('0') << problem.ref_image_id << "..." << std::endl;
    cudaSetDevice(0);
    std::stringstream result_path;
    result_path << output_folder << "/HFPMVS" << "/" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
    std::string result_folder = result_path.str();
    mkdir(result_folder.c_str(), 0777);

    HFPMVS hfpmvs;
    if (geom_consistency) {
        hfpmvs.SetGeomConsistencyParams(multi_geometrty);
    }
    if (hierarchy) {
        hfpmvs.SetHierarchyParams();
    }

    
    hfpmvs.InuputInitialization(dense_folder, output_folder, problems, idx);

    hfpmvs.CudaSpaceInitialization(dense_folder, output_folder, problem, region_cur);
    
    //debugger.s("Run Patch Match");
    hfpmvs.RunPatchMatch();
    //debugger.e();
    const int width = hfpmvs.GetReferenceImageWidth();
    const int height = hfpmvs.GetReferenceImageHeight();
    ...
    for (int col = 0; col < width; ++col) {
        for (int row = 0; row < height; ++row) {
            ...
            float4 plane_hypothesis = hfpmvs.GetPlaneHypothesis(center);
            depths(row, col) = plane_hypothesis.w;
            normals(row, col) = cv::Vec3f(plane_hypothesis.x, plane_hypothesis.y, plane_hypothesis.z);
            costs(row, col) = hfpmvs.GetCost(center);
        }
    }

    if (planar_prior) {
        std::cout << "Run Planar Prior Assisted PatchMatch MVS ..." << std::endl;
        //debugger.s("Run Planar create");
        hfpmvs.SetPlanarPriorParams();
    
        const cv::Rect imageRC(0, 0, width, height);
        std::vector<cv::Point> support2DPoints;

        hfpmvs.GetSupportPoints(support2DPoints);

        std::stringstream region_path;
        region_path << dense_folder << "/" << region_cur << "/" << problem.ref_image_id << "/region.png";
        cv::Mat region;
        region = imread(region_path.str(), cv::IMREAD_GRAYSCALE);
        cv::resize(region, region, cv::Size(width, height), 0, 0, cv::INTER_NEAREST);
        
        cv::Mat supportsGraph(width, height, CV_8UC1);
        for(int i = 0; i < height; i++) {
            for(int j = 0; j < width; j++) {
                supportsGraph.at<uint8_t>(i, j) = 0;
            }
        }

        int count = 0;
        for(int i = 0, length = support2DPoints.size(); i < length; i++) {
            cv::Point p = support2DPoints[i];
            float d = depths(p.y, p.x);
            float c = costs(p.y, p.x);
            if(region.at<uint8_t>(p.y, p.x) == 1) {
                count++;
            }

            supportsGraph.at<uint8_t>(p.y, p.x) = 255;
        }

        const auto triangles = hfpmvs.DelaunayTriangulation(imageRC, support2DPoints);
        cv::Mat refImage = hfpmvs.GetReferenceImage().clone();
        ...
        for (const auto triangle : triangles) {
            if (imageRC.contains(triangle.pt1) && imageRC.contains(triangle.pt2) && imageRC.contains(triangle.pt3)) {
                cv::Scalar triangleColor;

                int region1 = region.at<uint8_t>(triangle.pt1.y, triangle.pt1.x);
                int region2 = region.at<uint8_t>(triangle.pt2.y, triangle.pt2.x);
                int region3 = region.at<uint8_t>(triangle.pt3.y, triangle.pt3.x);                

                if(region1 == region2 && region2 == region3) {
                    triangleColor = cv::Scalar(0, 0, 255);
                }  else {
                    triangleColor = cv::Scalar(0, 255, 0);
                }

                cv::line(srcImage, triangle.pt1, triangle.pt2, triangleColor);
                cv::line(srcImage, triangle.pt1, triangle.pt3, triangleColor);
                cv::line(srcImage, triangle.pt2, triangle.pt3, triangleColor);
            }
        }
        ...
        for (const auto triangle : triangles) {
            if (imageRC.contains(triangle.pt1) && imageRC.contains(triangle.pt2) && imageRC.contains(triangle.pt3)) {
                ...
                bool is_same_region = (region1 == region2 && region2 == region3);
                ...
                if(is_same_region) {
                    ...
                    float4 n4 = hfpmvs.GetPriorPlaneParams(triangle, depths);
        ...
        //debugger.s("Run Planar Fix");
        cv::Mat errors = cv::Mat::zeros(height, width, CV_8UC1);
        for(int i = 0; i < width; i++) {
            for(int j = 0; j < height; j++) {
                if (mask_tri(j, i) > 0) {
                    float d = hfpmvs.GetDepthFromPlaneParam(planeParams_tri[mask_tri(j, i) - 1], i, j);
                    
                    if (d > hfpmvs.GetMaxDepth() && d < hfpmvs.GetMinDepth()) {
                        errors.at<uint32_t>(j, i) = 255;
                        mask_tri(j, i) = 0;
                    }
                }
            }
        ...
        std::cout << "Start find Region " << std::endl;
        hfpmvs.findRegionInitialization(planeParams_tri, mask_tri, errors);
        hfpmvs.RunFindSurRegion(mask_tri);
        ...
        for (int i = 0; i < width; ++i) {
            for (int j = 0; j < height; ++j) {
                if (mask_tri(j, i) > 0) {
                    ...
                    if (d <= hfpmvs.GetMaxDepth() && d >= hfpmvs.GetMinDepth()) {
                        priordepths(j, i) = d;
                        priornormals(j, i) = cv::Vec3f(plane_hypothesis.x, plane_hypothesis.y, plane_hypothesis.z);
                    }
                    else {
                        mask_tri(j, i) = 0;
                    }
        ...
        std::string normals_path = result_folder + "/normals_prior.dmb";
        writeNormalDmb(normals_path, priornormals);
        std::string plane_masks_path = result_folder + "/plane_masks.dmb";
        writeDepthDmb(plane_masks_path, mask_tri);

        //debugger.s("Planar Assist");
        hfpmvs.CudaPlanarPriorInitialization(planeParams_tri, mask_tri);
...
void JointBilateralUpsampling(const std::string &dense_folder, const std::string &output_folder, const Problem &problem, int hfpmvs_size)
{
    std::stringstream result_path;
    result_path << output_folder << "/HFPMVS" << "/" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
    std::string result_folder = result_path.str();
    ...
    const float factor_x = static_cast<float>(hfpmvs_size) / image_float.cols;
    const float factor_y = static_cast<float>(hfpmvs_size) / image_float.rows;
    ...
    RunJBU(output_folder, scaled_image_float, ref_depth, dense_folder, problem );
}

void RunFusion(std::string &dense_folder, std::string &output_folder, const std::vector<Problem> &problems, bool geom_consistency)
{
    size_t num_images = problems.size();
    std::string image_folder = dense_folder + std::string("/images");
    ...
    for (size_t i = 0; i < num_images; ++i) {
        ...
        std::stringstream result_path;
        result_path << output_folder << "/HFPMVS" << "/" << std::setw(8) << std::setfill('0') << problems[i].ref_image_id;
        std::string result_folder = result_path.str();
        std::string suffix = "/depths.dmb";
        if (geom_consistency) {
            suffix = "/depths_geom.dmb";
    ...
    std::string ply_path = output_folder + "/HFPMVS/HFPMVS_model.ply";
...
{
    if (argc < 2) {
        std::cout << "USAGE: HFPMVS dense_folder" << std::endl;
        return -1;
    }
    ...
    std::stringstream output_HFPMVS_folder;
    output_HFPMVS_folder << argv[2] << "/HFPMVS";
    mkdir(output_HFPMVS_folder.str().c_str(), 0777);
    ...
    std::stringstream output_log_folder;

    output_log_folder << argv[2] << "/log/";

    debugger.set_log_path(output_log_folder.str().c_str());

     debugger.s("runtime");

    std::cout << max_num_downscale << std::endl;
     ...
     if(true) { 
        while (max_num_downscale >= 0) {
            ...
            if (flag == 0) {
                ...
                planar_prior = true;
                for (size_t i = 0; i < num_images; ++i) {
                    ProcessProblem(dense_folder, output_folder, problems, i, geom_consistency, planar_prior, hierarchy, "region_1");
                }
                ...
                for (int geom_iter = 0; geom_iter < geom_iterations; ++geom_iter) {
                    ...
                    for (size_t i = 0; i < num_images; ++i) {
                        ProcessProblem(dense_folder, output_folder, problems, i, geom_consistency, planar_prior, hierarchy, "region_1", multi_geometry);
                    }
                }
            }
            else {
                ...
                // if(my_flag == 0) {
                //     my_flag++;
                //     region_path = "region_1_100";
                // } else {
                //     region_path = "region_100";
                // }

                region_path = "region_no_span";
                ...
                planar_prior = true;
                for (size_t i = 0; i < num_images; ++i) {
                    ProcessProblem(dense_folder, output_folder, problems, i, geom_consistency, planar_prior, hierarchy, region_path);
                }
                hierarchy = false;
                geom_consistency = true;
                planar_prior = false;
                ...
                for (int geom_iter = 0; geom_iter < geom_iterations; ++geom_iter) {
                    ...
                    for (size_t i = 0; i < num_images; ++i) {
                        ProcessProblem(dense_folder, output_folder, problems, i, geom_consistency, planar_prior, hierarchy, region_path, multi_geometry);
                    }
                }
            }
    ...
    if(false) {
        ...
        bool planar_prior = false;
        bool hierarchy = false;
        bool multi_geometry = true;

        for (size_t i = 0; i < num_images; ++i) {
            ProcessProblem(dense_folder, output_folder, problems, i, geom_consistency, planar_prior, hierarchy, "region_100_small", multi_geometry);
        }        
    }
    
    if(true) {
        geom_consistency = true;
        RunFusion(dense_folder, output_folder, problems, geom_consistency);
    }
